...
class Http2Preload implements ContainerAwareInterface
{
     ...
     * @return false|void
     */
    public function add(UriInterface $uri, string $type, string $fetchPriority = 'auto')
    {
        if (!$this->enable) {
            return;
        }
        ...
        $this->internalAdd($uri, $type, $this->extension($uri), $fetchPriority);
    }
    public function preload(UriInterface $uri, $type, $fontExt, $fetchPriority = ''): void
    {
        if ($this->validateUri($uri)) {
            $this->internalAdd($uri, $type, $fontExt, $fetchPriority);
        }
    }
     ...
     * @param string $type
     * @param string $fontExt
     * @param string $fetchPriority
     */
    private function internalAdd(UriInterface $uri, string $type, string $fontExt = '', string $fetchPriority = 'auto'): void
    {
        ...
        $preload = ['href' => (string) $RR_uri, 'as' => $type];
        ...
        $method = 'link';
        if ($this->cdn->isFileOnCdn($RR_uri) || UriComparator::isCrossOrigin($RR_uri) || Cache::isPageCacheEnabled($this->params, \true) && JCH_PRO && $this->params->get('pro_capture_cache_enable', '1') && !$this->params->get('pro_cache_platform', '0') || $fetchPriority != 'auto') {
            $method = 'html';
        }
    ...
    public function addAdditional(UriInterface $uri, string $type, string $fontExt, $fetchPriority = 'auto'): void
    {
        if ($this->enable) {
            $this->internalAdd($uri, $type, $fontExt, $fetchPriority);
        }
