 ...
 */
 defined( '_JEXEC' ) or die(); try { JHtml::_( version_compare( JVERSION, '3.3.0', 'lt' ) ? 'behavior.framework' : 'behavior.core' ); if ( AimySitemapCompatHelper::isJoomla3() ) { JHtml::_( 'formbehavior.chosen', 'select' ); } } catch ( Exception $e ) { } require_once( JPATH_COMPONENT . '/helpers/rights.php' ); $list_order = $this->escape( $this->state->get( 'list.ordering' ) ); $list_dir = $this->escape( $this->state->get( 'list.direction' ) ); $data_opts = array( 'list.attr' => array( 'class' => 'inputbox form-select' ) ); $filter_opts = array( 'list.attr' => array( 'class' => 'inputbox form-select' ), 'onchange' => 'this.form.submit();' ); if ( ! $this->allow_edit ) { $data_opts[ 'list.attr' ][ 'disabled' ] = 'disabled'; } AimySitemapCompatHelper::addInlineJavascript( 'jQuery( document ).ready(function()' . '{' . 'Joomla.submitbutton = function( task )' . '{' . 'if ( task == "urls.reset_index" )' . '{' . 'microalert.confirm( ' . json_encode( JText::_( 'AIMY_SM_MSG_CONFIRM_RESET_INDEX' ) ) . ',' . json_encode( JText::_( 'JYES' ) ) . ',' . json_encode( JText::_( 'JNO' ) ) . ',' . 'function( reset )' . '{' . 'if ( reset ) Joomla.submitform(task);' . '}' . ');' . 'return false;' . '}' . 'Joomla.submitform( task );' . '};' . 'AimySitemapAjaxInit( "' . JSession::getFormToken() .'" );' . '});' ); if ( empty( $this->items ) ) { AimySitemapCompatHelper::addInlineJavascript( 'jQuery( document ).ready(function()' . '{' . 'jQuery( "#toolbar-file button, #toolbar-trash button" )' . '.each(function()' . '{' . 'jQuery( this ).prop( "disabled", "disabled" );' . '});' . '});' ); } ?>

<div id="j-main-container" class="j-main-container aimy-main clearfix">

<form action="<?php
 echo JRoute::_( 'index.php?option=com_aimysitemap&view=urls' ); ?>" method="post" name="adminForm" id="adminForm"
    class="clearfix urls_search">
...
<div id="view-toolbar" class="btn-toolbar clearfix">
 ...
 echo "jQuery( '#filter_search' ).val( '' );", "jQuery( '#adminForm' ).submit();"; ?>"><i class="aimy-icon-cancel"></i></button>
    ...
    <div class="btn-group pull-left">
        <label for="filter_lang" class="element-invisible"><?php
 echo JText::_( 'AIMY_SM_FILTER_SELECT_LANG' ); ?></label>
        <?php
 echo JHtml::_( 'select.genericlist', $this->get_language_options( true ), 'filter_lang', $filter_opts, 'value', 'text', $this->state->get( 'filter.lang' ) ); ?>
    </div>
 ...
 echo JText::_( 'AIMY_SM_FILTER_SELECT_STATE' ); ?></label>
        <?php
 echo JHtml::_( 'select.genericlist', $this->get_state_options( true ), 'filter_state', $filter_opts, 'value', 'text', $this->state->get( 'filter.state' ) ); ?>
    </div>


    <div class="btn-group pull-left">
        <label for="filter_lock" class="element-invisible"><?php
 echo JText::_( 'AIMY_SM_FILTER_SELECT_LOCK' ); ?></label>
        <?php
 echo JHtml::_( 'select.genericlist', $this->get_lock_options( true ), 'filter_lock', $filter_opts, 'value', 'text', $this->state->get( 'filter.lock' ) ); ?>
    </div>
...
<div id="j-main-container" class="clearfix">
 ...
 echo JHtml::_( 'grid.sort', 'AIMY_SM_URL_LBL', 'a.url', $list_dir, $list_order ); ?>
                ...
                <th class="lang">
                    <?php
 echo JHtml::_( 'grid.sort', 'AIMY_SM_LANGUAGE_LBL', 'a.lang', $list_dir, $list_order ); ?>
                </th>
                <th class="state">
                    <?php
 echo JHtml::_( 'grid.sort', 'AIMY_SM_STATE_LBL', 'a.state', $list_dir, $list_order ); ?>
                ...
                <th class="lock">
                    <?php
 echo JHtml::_( 'grid.sort', 'AIMY_SM_LOCK_LBL', 'a.lock', $list_dir, $list_order ); ?>
                </th>
                <th class="priority">
                    <?php
 ...
 echo JText::_( 'AIMY_SM_TITLE_LBL' ), ': "', htmlspecialchars( $item->title, ENT_COMPAT, 'UTF-8', false ), '"'; ?></small>
                ...
                <td class="lang">
                <?php
 echo JHtml::_( 'select.genericlist', $this->get_language_options(), 'lang', $data_opts, 'value', 'text', $item->lang, "lang-$i" ); ?>
                </td>
 ...
 if ( $this->allow_edit ) { echo '<a href="', JRoute::_( 'index.php?option=com_aimysitemap&' . 'task=url.toggle_state&' . JSession::getFormToken() . '=1&' . 'id=' . (int) $item->id ), '" ', 'title="', htmlspecialchars( JText::_( $item->state != 1 ? 'AIMY_SM_ACTIVATE_LBL' : 'AIMY_SM_DEACTIVATE_LBL' ) ), '" ', 'data-title-activate="', JText::_( 'AIMY_SM_ACTIVATE_LBL' ), '" ', 'data-title-deactivate="', JText::_( 'AIMY_SM_DEACTIVATE_LBL' ), '"', '>'; } echo '<span class="aimy-icon-', ( $item->state == 1 ? 'ok' : 'cancel' ), ( $this->allow_edit ? ' btn btn-micro' : '' ), '"></span>'; if ( $this->allow_edit ) { echo '</a>'; } ?>
                ...
                <td class="lock" data-initial-lock="<?php
 echo $item->lock; ?>">
                    <?php
 if ( $this->allow_edit ) { echo '<a href="', JRoute::_( 'index.php?option=com_aimysitemap&' . 'task=url.toggle_lock&' . JSession::getFormToken() . '=1&' . 'id=' . (int) $item->id ), '" ', 'title="', htmlspecialchars( JText::_( $item->lock != 1 ? 'AIMY_SM_DO_LOCK_LBL' : 'AIMY_SM_DO_UNLOCK_LBL' ) ), '" ', 'data-title-lock="', JText::_( 'AIMY_SM_DO_LOCK_LBL' ), '" ', 'data-title-unlock="', JText::_( 'AIMY_SM_DO_UNLOCK_LBL' ), '"', '>'; } echo '<span class="aimy-icon-', ( $item->lock == 1 ? 'lock' : 'unlock' ), ( $this->allow_edit ? ' btn btn-micro' : '' ), '"></span>'; if ( $this->allow_edit ) { echo '</a>'; } ?>
                </td>


                <td class="priority">
                <?php
 echo JHtml::_( 'select.genericlist', $this->get_priority_options(), 'priority', $data_opts, 'value', 'text', number_format( $item->priority, 1 ), "priority-$i" ); ?>
                </td>
 ...
 if ( $item->mtime == 0 ) { echo "&mdash;"; } else { echo date( JText::_( 'AIMY_SM_DATE_FMT' ), $item->mtime ); } ?>
                ...
                <?php
 echo JHtml::_( 'select.genericlist', $this->get_changefreq_options(), 'changefreq', $data_opts, 'value', 'text', $item->changefreq, "changefreq-$i" ); ?>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
