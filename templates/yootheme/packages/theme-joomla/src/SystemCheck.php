...
class SystemCheck extends BaseSystemCheck
{
    ...
    public function getRequirements()
    {
        ...
        if (!PluginHelper::isEnabled('installer', 'yootheme')) {
            $res[] = trans(
                'The YOOtheme Installer Plugin, which enables <span class="uk-text-nowrap">1-click</span> updates, is disabled or missing. Enable the <a href="index.php?option=com_plugins&view=plugins" target="_blank">YOOtheme Installer</a> plugin or reinstall the theme package.',
            );
        }
