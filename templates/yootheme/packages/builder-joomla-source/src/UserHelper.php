...
class UserHelper
{
    ...
    public static function getAuthorList()
    {
        ...
        $db = app(DatabaseDriver::class);
        $query = sprintf(
            'SELECT DISTINCT(user_id) as value, u.name as text
    ...
    protected static function getModel()
    {
        if (version_compare(JVERSION, '4.0', '<')) {
            BaseDatabaseModel::addIncludePath(
                JPATH_ADMINISTRATOR . '/components/com_users/models/',
            );
    ...
    protected static function getContactModel()
    {
        if (version_compare(JVERSION, '4.0', '<')) {
            BaseDatabaseModel::addIncludePath(
                JPATH_ADMINISTRATOR . '/components/com_contact/models/',
            );

            return BaseDatabaseModel::getInstance('contacts', 'ContactsModel', [
                'ignore_request' => true,
            ]);
        }
        ...
        return Factory::getApplication()
            ...
            ->getMVCFactory()
            ->createModel('contacts', 'administrator', ['ignore_request' => true]);
    }
}
